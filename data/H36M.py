...
def generate_heatmaps_and_fill_dataset(entry, model, split, datadir, hard_subset_list):
    ...
    for subject in subject_list:
        ...
        for sequence in auto.tqdm(sorted(pose_2d_dir.iterdir()), desc='Sequence '+subject):
            ...
            for idx, (img, pose) in enumerate(zip(cap, poses_2d)):
                # Start from frame 3 as in
                # https://github.com/twehrbein/Probabilistic-Monocular-3D-Human-Pose-Estimation-with-Normalizing-Flows/blob/main/create_data.py
                if (idx + 1) % 4 != 0:
                    continue
